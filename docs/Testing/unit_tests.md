...
![Unit test resultaten](./Screenshots/UnitTests.png)
...
### SecurityTests (6 testen)

Valideert beveiligingsinvarianten die **nooit** gebroken mogen worden, ongeacht toekomstige wijzigingen.

| Scenario                                             | Getest? | Verwacht resultaat                                         |
| ---------------------------------------------------- | ------- | ---------------------------------------------------------- |
| Alle repository-aanroepen gefilterd op `TenantId`    | ✅      | Cross-tenant data lekkage onmogelijk                       |
| CANCELLED lookup gefilterd op `TenantId`             | ✅      | Tenant kan nooit een andere tenant's appointment annuleren |
| API-key validatie loopt altijd via `IHashingService` | ✅      | Plain-text vergelijking verboden                           |
| Ongeldige API-key geweigerd ook als tenant bestaat   | ✅      | `ErrorType.Unauthorized`                                   |
| Provider credentials gedecrypteerd vóór gebruik      | ✅      | Versleutelde waarden bereiken nooit de provider            |
| Decryptie-fout → dispatch afgebroken                 | ✅      | `ErrorType.Failure`, geen gedeeltelijke verzending         |
...
| Aspect               | Bevinding                                                                                                                                                                                    |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Betrouwbaarheid**  | 59 van 59 testen slagen. Alle kritieke paden, foutsituaties en randgevallen zijn gedekt. Branch coverage van ~89% toont aan dat nagenoeg alle beslissingstakken getest zijn.                 |
